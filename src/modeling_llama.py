...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if self.config.dump_tensors and hidden_states.shape[1] > 1:
            query_dir = "tensors/query_states"
            key_dir = "tensors/key_states"
            scaling_dir = "tensors/scaling"

            for directory in (query_dir, key_dir, scaling_dir):
                os.makedirs(directory, exist_ok=True)

            torch.save(query_states, f"{query_dir}/{self.layer_idx}.pt")
            torch.save(key_states, f"{key_dir}/{self.layer_idx}.pt")
            torch.save(self.scaling, f"{scaling_dir}/{self.layer_idx}.pt")
